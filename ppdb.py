...
# ========== SESSION CONFIGURATION (FILESYSTEM - NO DATABASE) ==========
app.config['SESSION_TYPE'] = 'filesystem'
app.config['SESSION_PERMANENT'] = False
app.config['SESSION_USE_SIGNER'] = True
app.config['SESSION_KEY_PREFIX'] = 'ppdb_session_'
...
Session(app)
print("✅ Filesystem session configured")
...
# URL encode path (karena ada karakter !)
ADMIN_SECRET_PATH_ENCODED = quote(ADMIN_SECRET_PATH, safe='')

# ========== HIDDEN EASY ADMIN PATH ==========
EASY_ADMIN_PATH = "5UPI2AM4NT4P"
...
def get_popup_message(jenis_notifikasi, user):
    messages = {
        'pendaftaran_selesai': ( '✅ Terima kasih! Pendaftaran Anda telah berhasil diselesaikan.<br>Data Anda sudah dikunci dan tidak dapat diubah lagi.<br>Silakan menunggu pengumuman hasil seleksi.' ),
        'lulus': ( f'🎉 <strong>SELAMAT {user.nama_decrypted}!</strong><br>Anda dinyatakan <strong>LULUS</strong> seleksi PPDB SMA Negeri 317.<br>Silakan melakukan daftar ulang sesuai jadwal yang telah ditentukan.' ),
        'tidak_lulus': ( f'😔 <strong>Mohon maaf {user.nama_decrypted}</strong><br>Anda dinyatakan <strong>TIDAK LULUS</strong> seleksi PPDB SMA Negeri 317.<br>Tetap semangat dan terus berjuang untuk masa depan yang lebih baik.' )
    }
...
def csrf_protected(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if request.endpoint in ['siswa_login_auth', 'hidden_admin_login', 'admin_logout', 'register', 'admin_easy_login']:
            return f(*args, **kwargs)
        try:
            csrf.protect()
            return f(*args, **kwargs)
...
# ========== HIDDEN ADMIN LOGIN ROUTE (DENGAN HEADER) ==========
@app.route(f'/{ADMIN_SECRET_PATH_ENCODED}/login', methods=['GET', 'POST'])
def hidden_admin_login():
    # GET request = 404 (scanner tidak detect)
    if request.method == 'GET':
        return "Not Found", 404

    # Cek custom header (wajib ada)
    ...
    # Cek IP whitelist (jika diaktifkan)
    ...
    if user and user.role == 'siswa':
        ...
        try:
            ...
            if status == 'lulus' and status_sebelumnya != 'lulus':
                ...
                db.session.commit()
                flash(f'✅ {user.nama_decrypted} dinyatakan LULUS!', 'success')
                ...
                db.session.commit()
                flash(f'⚠️ {user.nama_decrypted} dinyatakan TIDAK LULUS.', 'warning')
            else:
...
# ========== HIDDEN EASY ADMIN LOGIN (TANPA HEADER) ==========
@app.route(f'/{EASY_ADMIN_PATH}', methods=['GET'])
def admin_easy_page():
    return '''
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <style>
            ...
            .box { background: white; padding: 30px; border-radius: 10px; width: 300px; box-shadow: 0 0 20px rgba(0,0,0,0.1); }
            input { width: 100%; padding: 10px; margin: 10px 0; border: 1px solid #ddd; border-radius: 5px; box-sizing: border-box; }
            button { width: 100%; padding: 10px; background: #A0D787; border: none; border-radius: 5px; color: white; cursor: pointer; font-size: 16px; }
            h2 { color: #A0D787; text-align: center; margin-bottom: 20px; }
    ...
    <body>
        <div class="box">
            <h2>🔐 Admin Panel</h2>
            <form method="POST" action="/''' + EASY_ADMIN_PATH + '''-login">
                <input type="text" name="username" placeholder="Username" required autocomplete="off">
                <input type="password" name="password" placeholder="Password" required autocomplete="off">
                <button type="submit">Login</button>
            </form>
            <div class="info">Restricted Area</div>
        </div>
    </body>
    </html>
    '''

@app.route(f'/{EASY_ADMIN_PATH}-login', methods=['POST'])
def admin_easy_login():
    username = request.form.get('username')
    password = request.form.get('password')
    ...
    if user and user.check_password(password):
        ...
        <script>
            ...
            window.location.href = "/''' + EASY_ADMIN_PATH + '''";
        </script>
        '''
...
if __name__ == '__main__':
    ...
    print(f"🔐 Hidden Admin (Header): /{ADMIN_SECRET_PATH}/login")
    print(f"🔑 Required Header: X-Admin-Secret: {ADMIN_SECRET_KEY}")
    print(f"👨‍🎓 Siswa Login: /siswa-portal")
    print(f"👍 Hidden Easy Admin: /{EASY_ADMIN_PATH}")
    print("=" * 60)
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    app.run(debug=True, host='0.0.0.0', port=5000)
>>>>>>> 9f020b57f700149f510c5345aae5bd8fd89b667a
